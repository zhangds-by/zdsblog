...
import com.zhangds.zdsblog.common.model.interfaces.Base;
import com.zhangds.zdsblog.common.model.interfaces.BaseId;
import com.zhangds.zdsblog.common.model.interfaces.Filter;
import com.zhangds.zdsblog.common.model.interfaces.GetId;
...
public class CommonUtils {
    ...
    public static boolean notEmptyOrBlank(Object obj) {
    	...
    	if(obj instanceof CharSequence){
    		return ObjectUtil.isNotEmpty((CharSequence)obj);
     ...
     * date 2017年11月23日
     */
    public static <K extends Serializable, T extends BaseId<K>> Set<K> getIdSet(Collection<T> entityList){
     ...
     * date 2017年11月23日
     */
    public static <K extends Serializable, T extends BaseId<K>> Map<K, T> getIdMap(Collection<T> entityList){
...
//    		log.error(String.format("[%s]字符串解析成Double失败", str));
	...
	public static <T, K> T convert(K source, Class<T> clazz){
		T toValue = Base.objectMapper.convertValue(source, clazz);
		return toValue;
	} 
    
	public static <T, K> List<T> convertList(List<K> fromList, Class<T> clazz){
		List<T> toList = Lists.newArrayList();
		for(K k : CommonUtils.nullToEmpty(fromList)){
			toList.add(convert(k, clazz));
		}
		return toList;
	} 
...
//	}
	...
	public static Map<String, Double> sumToMap(Map<String, Double> map, String key, Double value){
		if(map != null && value != null){
			Double sum = NumberUtil.add(map.get(key), value);
			map.put(key, sum);
		}
		return map;
	}
	...
	public static String replaceDomin(String url, String domain){
		if(StrUtil.isBlank(url) || StrUtil.isBlank(domain) ){
			return url; 
		}
	 ...
	 * @date 2019年9月19日
	 */
    public static <K extends Serializable, V extends Serializable> void distinct(List<V> list, GetId<K, V> getId){
    	final Set<K> set = Sets.newHashSet();
    	CommonUtils.filter(list, t -> set.add(getId.getId(t)));
	}
